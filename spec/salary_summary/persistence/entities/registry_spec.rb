      ...
      describe Registry do
        ...
        describe '#add entity' do
          ...
          it "doesn't add another object with same ID and class when already present" do
            ...
            another_salary = SalarySummary::Entities::Salary.new(id: id, amount: 1400.0, period: Date.parse('07/09/2017'))
          ...
          it 'adds two objects of same entity and different IDs' do
            ...
            other_salary = SalarySummary::Entities::Salary.new(id: another_id, amount: 1400.0, period: Date.parse('07/09/2017'))

            subject.add(other_salary)

            expect(subject.entities.size).to eql 2
            expect(subject.entities).to have_key("salarysummary::entities::salary>>#{id}")
            expect(subject.entities).to have_key("salarysummary::entities::salary>>#{another_id}")
            ...
            expect(
              subject.entities.dig("salarysummary::entities::salary>>#{another_id}")
        ...
        describe '#get class_name, id' do
          ...
          it 'returns nil if no object is found' do
            expect(
              subject.get('SalarySummary::Entities::Salary', id)
            ).to be_nil
          end
        end
            ...
            expect(
              subject.include?(
                SalarySummary::Entities::Salary.new(
                  id: id, amount: 1400.0, period: Date.parse('07/09/2017')
                )
              )
            ).to be false
          end
        ...
        describe '#delete entity' do
          ...
          it 'returns nil if no entity was removed' do
            expect(
              subject.delete(SalarySummary::Entities::Salary.new(id: id))
            ).to be_nil
          end
        end
